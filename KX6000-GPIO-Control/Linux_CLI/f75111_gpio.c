...
// 功能选择寄存器
#define GPIO_SET1_FUNC_SEL_OFFSET 0xF0
#define GPIO_SET2_FUNC_SEL_OFFSET 0xF1
...
static void dump_registers(void) {
    ...
    for (reg = 0x00; reg <= 0x30; reg++) {
        value = inb(SMBUS_BASE + reg);
        printk(KERN_INFO "Reg 0x%02X: 0x%02X\n", reg, value);
    }
    
    // 添加功能选择寄存器的打印
    value = read_smbus_byte(GPIO_SET1_FUNC_SEL_OFFSET);
    printk(KERN_INFO "Reg 0xF0 (SET1_FUNC_SEL): 0x%02X\n", value);
    
    value = read_smbus_byte(GPIO_SET2_FUNC_SEL_OFFSET);
    printk(KERN_INFO "Reg 0xF1 (SET2_FUNC_SEL): 0x%02X\n", value);
}
...
static long f75111_ioctl(struct file *file, unsigned int cmd, unsigned long arg) {
    ...
    switch (cmd) {
        case IOCTL_SET_DIRECTION: {
            ...
            // 设置所有引脚为GPIO模式 (0=GPIO, 1=特殊功能)
            //write_smbus_byte(GPIO_SET1_FUNC_SEL_OFFSET, 0x00);
            //write_smbus_byte(GPIO_SET2_FUNC_SEL_OFFSET, 0x00);
		...
		write_smbus_byte(GPIO_SET1_OUTPUT_DATA_OFFSET, 0x00);
              ...
              //  write_smbus_byte(0x13, 0xFF); // SET1 上拉使能
                ...
                // 禁用上拉电阻
                //write_smbus_byte(0x13, 0x00); // SET1 禁用上拉
                //write_smbus_byte(0x23, 0x00); // SET2 禁用上拉
        ...
        case IOCTL_SET_OUTPUT: {
            u8 reg, mask, state, cur_value, new_value, read_back;
            
            if (copy_from_user(data, (int __user *)arg, sizeof(data)))
                return -EFAULT;
                
            printk(KERN_INFO "Setting output: set=%d, mask=0x%02X, state=%d\n",
                   data[0], data[1], data[2]);
                   
            reg = (data[0] == 0) ? GPIO_SET1_OUTPUT_DATA_OFFSET : GPIO_SET2_OUTPUT_DATA_OFFSET;
            mask = (u8)data[1];
            state = (u8)data[2];
            
            // 读取当前输出值
            cur_value = read_smbus_byte(reg);
            if (state) {
                new_value = cur_value | mask;
            } else {
                new_value = cur_value & ~mask;
            }
            
            write_smbus_byte(reg, new_value);
            
            // 添加延迟确保输出稳定
            udelay(1000);
            
            // 验证写入
            read_back = read_smbus_byte(reg);
